...
namespace Presentation.Controllers
{
    ...
    public class FilesController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetServiceImageAsync(string blobName, [FromQuery] bool download = false)
        {
            try
            {
                _logger.LogInformation("Requesting technician file: {BlobName}", blobName);
                ...
                // Get file properties first to check content type
                var properties = await _fileService.GetFilePropertiesAsync(blobName, "services-documents");
                var fileStream = await _fileService.GetFileStreamAsync(blobName, "services-documents");
